...
def print_response_data(response: httpx.Response, data: Any = None, success_status: int | None = None) -> None:
...
async def get_cohere_embeddings(
    ...
    print(f"{YELLOW}Generating embeddings with Cohere for {len(texts)} text(s) (model: {COHERE_MODEL}, type: {input_type})...{NC}")
    try:
...
async def create_library(api_client: httpx.AsyncClient, name: str, dimension: int) -> str:
    ...
    response = await api_client.post(f"{API_V1_PREFIX}/libraries/", json=library_payload)
...
async def add_chunks_in_bulk(
    ...
    print_section(f"Adding {len(document_texts)} Chunks in Bulk to Library: {library_id}")
    chunks_to_create = []
    ...
    response = await api_client.post(f"{API_V1_PREFIX}/libraries/{library_id}/chunks/bulk", json=bulk_payload)
...
async def trigger_library_index_build(api_client: httpx.AsyncClient, library_id: str) -> None:
...
async def search_library(
    ...
    response = await api_client.post(f"{API_V1_PREFIX}/libraries/{library_id}/search", json=search_payload)
...
async def run_cohere_example_flow():
    ...
    if not COHERE_API_KEY:
        print(f"{RED}CRITICAL ERROR: COHERE_API_KEY environment variable not set. This example cannot run.{NC}")
        return
    ...
    async with cohere.AsyncClient(COHERE_API_KEY) as cohere_client, \
        ...
        try:
            ...
            library_id = await create_library(api_client, temp_library_name, EMBEDDING_DIMENSION)

            document_embeddings = await get_cohere_embeddings(cohere_client, texts_to_index, input_type="search_document")
            
            await add_chunks_in_bulk(api_client, library_id, texts_to_index, document_embeddings)
            
            await trigger_library_index_build(api_client, library_id)
            
            print(f"{YELLOW}Waiting briefly to allow potential indexing or propagation...{NC}")
            await asyncio.sleep(2) 

            query_embedding_list = await get_cohere_embeddings(cohere_client, [search_query_text], input_type="search_query")
            query_embedding = query_embedding_list[0]
            
            search_results = await search_library(api_client, library_id, query_embedding, k_results=3)
            ...
            err_body_msg = co_err.body.get('message') if hasattr(co_err, 'body') and isinstance(co_err.body, dict) else 'N/A'
